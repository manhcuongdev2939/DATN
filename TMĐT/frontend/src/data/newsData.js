const THEMES = [
  ...
  {
    category: "Khuyến mãi",
    img: "https://images.unsplash.com/photo-1545239351-1141bd82e8a6?w=1200&q=80&auto=format&fit=crop",
  },
];
...
export const sampleArticles = Array.from({ length: 100 }).map((_, i) => {
  ...
  const title = `${category} - ${projectSnippets[
    i % projectSnippets.length
  ].slice(0, 40)}${idx}`;
  return {
    id: `sample-${idx}`,
    title,
    ...
    excerpt: `${
      projectSnippets[i % projectSnippets.length]
    } Tin chi tiết số ${idx} dành cho khách hàng TechStore.`,
    content: `${
      projectSnippets[i % projectSnippets.length]
