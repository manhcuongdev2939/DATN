...
export default function RegisterPage({ onSuccess }) {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      if (result?.error) {
        // Nếu lỗi liên quan đến email đã tồn tại, hiển thị lỗi ngay dưới trường email
  ...
  const getPasswordStrengthLabel = () => {
    ...
    if (passwordStrength === 3)
      return { text: "Trung bình", color: "bg-yellow-500" };
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-5">
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={
                    verifyingOTP || !formData.otp || formData.otp.length !== 6
                  }
                  ...
                  <div className="mt-2">
                    ...
                    {strengthLabel.text && (
                      <p className="text-xs text-gray-600">
                        Độ mạnh:{" "}
                        <span
                          className={`font-medium ${strengthLabel.color.replace(
                            "bg-",
                            "text-"
                          )}`}
                        >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (fieldErrors.So_dien_thoai)
                        validateField("So_dien_thoai", e.target.value);
                    }}
                    onBlur={() =>
                      validateField("So_dien_thoai", formData.So_dien_thoai)
                    }
                    className={`block w-full pl-10 pr-3 py-3 border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent sm:text-sm ${
